...
namespace vicon_bridge
{
  ...
  void ViconBridge::process_frame(rclcpp::Time &grab_time)
  {
    ...
    // RCLCPP_INFO(get_logger(), "FIRST: %zu, LAST: %zu, CURR: %zu, DROP: %zu, TOTAL: %zu", first_frame_number_, last_frame_number_, frame_number, drop_count_, frame_count_);
    // RCLCPP_INFO(get_logger(), "Update rate HZ: %f", update_rate_hz_);
    ...
    if (success)
    {
      ...
      if (lock.owns_lock())
      {
        ...
        else
        {
          ...
          lock.unlock(); // TODO: fix locks, here and when is_ready is set
  ...
  bool ViconBridge::get_twist_stamped_msg(geometry_msgs::msg::TwistStamped &msg, const rclcpp::Time &frame_time, const std::string &subject_name, const std::string &segment_name)
  {
    struct PrevState
    {
      rclcpp::Time stamp;
      tf2::Vector3 p_m;    // in meters, world frame
      tf2::Quaternion q_w; // world orientation
      bool valid{false};
    };
    // static buffering to store previous state (keyed by subject/segment)
    static std::unordered_map<std::string, PrevState> prev;

    // body frame linear and angular velocities
    auto res_quat = client_.GetSegmentGlobalRotationQuaternion(subject_name, segment_name);
    auto res_trans = client_.GetSegmentGlobalTranslation(subject_name, segment_name);

    if (res_quat.Result != Result::Success || res_trans.Result != Result::Success)
    {
      RCLCPP_WARN(get_logger(), "getTwist failed for %s/%s",
                  subject_name.c_str(), segment_name.c_str());
      return false;
    }
    if (res_quat.Occluded || res_trans.Occluded)
    {
      RCLCPP_WARN(get_logger(), "Velocity occluded for %s/%s",
                  subject_name.c_str(), segment_name.c_str());
      return false;
    }

    const std::string key = subject_name + "/" + segment_name;

    tf2::Vector3 p_curr(res_trans.Translation[0] / 1000.0, res_trans.Translation[1] / 1000.0, res_trans.Translation[2] / 1000.0);
    tf2::Quaternion q_curr(res_quat.Rotation[0], res_quat.Rotation[1], res_quat.Rotation[2], res_quat.Rotation[3]);
    q_curr.normalize();

    auto it = prev.find(key);
    if (it == prev.end() || !it->second.valid)
    {
      prev[key] = PrevState{frame_time, p_curr, q_curr, true};
      return false; // no velocity on first call
    }

    const PrevState &pr = it->second;
    double dt = (frame_time - pr.stamp).seconds();
    if (dt <= 0.0)
      return false;

    // finite difference of position, to get velocity in world frame
    tf2::Vector3 v_world = (p_curr - pr.p_m) / dt;

    // angular velocity in world frame: from relative quaternion's axis-angle / dt, then rotate to world frame
    tf2::Quaternion dq = pr.q_w.inverse() * q_curr;
    dq.normalize();
    double s = std::sqrt(1 - dq.w() * dq.w());
    double angle = 2.0 * std::atan2(std::sqrt(dq.x() * dq.x() + dq.y() * dq.y() + dq.z() * dq.z()), std::fabs(dq.w()));
    if (angle > M_PI)
      angle -= 2.0 * M_PI;

    tf2::Vector3 axis = (s < 1e-9 || std::fabs(angle) < 1e-9) ? tf2::Vector3(0, 0, 0)
                                                              : tf2::Vector3(dq.x() / s, dq.y() / s, dq.z() / s);
    tf2::Vector3 w_body = axis * (angle / dt);
    tf2::Matrix3x3 R_curr(q_curr);
    tf2::Vector3 w_world = R_curr * w_body;

    // output message
    msg.header.stamp = frame_time;
    msg.header.frame_id = (tf_namespace_ + "/" + world_frame_id_);
    msg.twist.linear.x = v_world.x();
    msg.twist.linear.y = v_world.y();
    msg.twist.linear.z = v_world.z();
    msg.twist.angular.x = w_world.x();
    msg.twist.angular.y = w_world.y();
    msg.twist.angular.z = w_world.z();
